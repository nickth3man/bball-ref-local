...
class Team(BaseModel):
    ...
    abbreviation: str = Field(
        description="Team abbreviation (e.g., 'LAL', 'BOS')", min_length=2, max_length=3
    )
    nickname: str = Field(description="Team nickname (e.g., 'Lakers', 'Celtics')")
    city: str = Field(description="City where the team is based")
    state: str | None = Field(default=None, description="State where the team is based")
    year_founded: int | None = Field(default=None, description="Year the team was founded", ge=1946)
    arena: str | None = Field(default=None, description="Name of the team's home arena")
    owner: str | None = Field(default=None, description="Team owner or ownership group")
    general_manager: str | None = Field(default=None, description="Team's general manager")
    head_coach: str | None = Field(default=None, description="Team's head coach")
    conference: str = Field(
        description="Conference (Eastern or Western)", pattern=r"^(Eastern|Western)$"
    )
    division: str = Field(
