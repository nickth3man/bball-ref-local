...
async def list_games(
    ...
    if is_htmx_request(request):
        return HTMLResponse(
            content=f"<!-- game_list.html exists but not yet integrated: {len(games)} games -->"
        )
...
async def get_todays_games(request: Request) -> dict[str, Any] | HTMLResponse:
    ...
    if is_htmx_request(request):
        return HTMLResponse(
            content=f"<!-- game_list.html exists but not yet integrated: {len(games)} games for today -->"
        )
...
async def get_game_box_score(
    ...
    if is_htmx_request(request):
        return HTMLResponse(
            content=f"<!-- box_score.html exists but not yet integrated: {game_id} -->"
        )
