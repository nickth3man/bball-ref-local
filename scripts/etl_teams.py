...
def run_etl() -> dict:
    ...
    try:
        ...
        logger.error(f"Teams ETL API error: {e}")
    except DatabaseError as e:
        result["status"] = "failed"
        result["error"] = f"Database error during loading: {e}"
        logger.error(f"Teams ETL database error: {e}")
    except DataTransformationError as e:
        result["status"] = "failed"
        result["error"] = f"Data transformation error: {e}"
        logger.error(f"Teams ETL transformation error: {e}")
    except ETLError as e:
        result["status"] = "failed"
        result["error"] = f"ETL error: {e}"
        logger.error(f"Teams ETL error: {e}")
    except (ValueError, TypeError) as e:
        result["status"] = "failed"
        result["error"] = f"Data validation error: {e}"
        logger.error(f"Teams ETL validation error: {e}")
