...
Query naming convention: VALIDATE_{CATEGORY}_{SPECIFIC_CHECK}
"""
...
class ValidationCategory(Enum):
    """Categories of validation queries."""
...
class SQLValidators:
    """Container for all validation SQL queries.

    This class organizes validation queries by category for easier
    maintenance and discovery. All queries are defined as class
    constants with consistent naming conventions.
    """
    ...
    VALIDATE_POINTS_CALCULATION_GAME: Final[str] = """
    SELECT 
        player_id,
        game_id,
        points,
    ...
    VALIDATE_POINTS_CALCULATION_SEASON: Final[str] = """
    SELECT 
        player_id,
        season,
        points,
    ...
    VALIDATE_GAME_WINNERS: Final[str] = """
    SELECT 
        game_id,
        home_team_id,
        away_team_id,
    ...
    VALIDATE_FOREIGN_KEYS_PLAYERS: Final[str] = """
    SELECT 
    ...
    VALIDATE_FOREIGN_KEYS_TEAMS: Final[str] = """
    SELECT 
        pgs.player_team_city || ' ' || pgs.player_team_name as team_identifier,
        COUNT(*) as orphan_count
    FROM player_game_statistics pgs
    LEFT JOIN team_abbreviations t
        ON pgs.player_team_city = t.team_city AND pgs.player_team_name = t.team_name
    WHERE t.team_id IS NULL
    GROUP BY pgs.player_team_city, pgs.player_team_name
    ORDER BY orphan_count DESC;
    """

    VALIDATE_FOREIGN_KEYS_GAMES: Final[str] = """
    SELECT 
        pgs.game_id,
        COUNT(*) as orphan_count
    FROM player_game_statistics pgs
    ...
    VALIDATE_FOREIGN_KEYS_SEASONS: Final[str] = """
    SELECT 
        pss.season,
        COUNT(*) as orphan_count
    FROM player_season_totals pss
    LEFT JOIN games_historical g
        ON CAST(SUBSTRING(CAST(g.game_id AS VARCHAR), 1, 4) AS INTEGER) = pss.season
    WHERE g.game_id IS NULL
    GROUP BY pss.season
    ORDER BY orphan_count DESC;
    """
    ...
    VALIDATE_DUPLICATE_PLAYERS: Final[str] = """
    SELECT 
        player_id,
    ...
    VALIDATE_DUPLICATE_GAMES: Final[str] = """
    SELECT 
        game_id,
        COUNT(*) as duplicate_count
    ...
    VALIDATE_DUPLICATE_PLAYER_GAMES: Final[str] = """
    SELECT 
    ...
    VALIDATE_SEASON_RANGES: Final[str] = """
    SELECT 
        season,
    ...
    VALIDATE_PLAYER_CAREER_SEASONS: Final[str] = """
    SELECT 
        player_id,
        player as full_name,
        draft_year,
    ...
    VALIDATE_STAT_PERCENTAGES_SEASON: Final[str] = """
    SELECT 
        player_id,
        season,
    ...
    VALIDATE_STAT_PERCENTAGES_GAME: Final[str] = """
    SELECT 
        person_id as player_id,
        game_id,
        field_goal_pct as fg_pct,
    ...
    VALIDATE_GAME_SCORES: Final[str] = """
    SELECT 
        game_id,
        home_team_id,
        away_team_id,
    ...
    VALIDATE_GAME_QUARTER_SUMS: Final[str] = """
    SELECT 
        game_id,
    ...
    VALIDATE_PLAYER_AGES: Final[str] = """
    SELECT 
        p.player_id,
    ...
    VALIDATE_PLAYER_CAREER_SPAN: Final[str] = """
    SELECT 
        player_id,
        player as full_name,
        birth_date,
    ...
    VALIDATE_TEAM_SEASON_CONTINUITY: Final[str] = """
    ...
    VALIDATE_TEAM_ACTIVE_YEARS: Final[str] = """
    SELECT 
        t.team_id,
    ...
    @classmethod
    def get_query(cls, name: str) -> str | None:
        """Get a validation query by name.

        Args:
            name: The query constant name (e.g., 'VALIDATE_GAME_WINNERS')
    ...
    @classmethod
    def list_queries(cls) -> dict[str, str]:
        """List all available validation queries.
        ...
        return {
            ...
            for attr in dir(cls)
            if attr.startswith("VALIDATE_") and isinstance(getattr(cls, attr), str)
        }


# Backward-compatible aliases for existing code
VALIDATE_POINTS_CALCULATION = SQLValidators.VALIDATE_POINTS_CALCULATION_GAME
VALIDATE_POINTS_CALCULATION_SEASON = SQLValidators.VALIDATE_POINTS_CALCULATION_SEASON
VALIDATE_GAME_WINNERS = SQLValidators.VALIDATE_GAME_WINNERS
VALIDATE_GAME_TIES = SQLValidators.VALIDATE_GAME_TIES
VALIDATE_FOREIGN_KEYS_PLAYERS = SQLValidators.VALIDATE_FOREIGN_KEYS_PLAYERS
VALIDATE_FOREIGN_KEYS_TEAMS = SQLValidators.VALIDATE_FOREIGN_KEYS_TEAMS
VALIDATE_FOREIGN_KEYS_GAMES = SQLValidators.VALIDATE_FOREIGN_KEYS_GAMES
VALIDATE_FOREIGN_KEYS_SEASONS = SQLValidators.VALIDATE_FOREIGN_KEYS_SEASONS
VALIDATE_DUPLICATE_PLAYERS = SQLValidators.VALIDATE_DUPLICATE_PLAYERS
VALIDATE_DUPLICATE_GAMES = SQLValidators.VALIDATE_DUPLICATE_GAMES
VALIDATE_DUPLICATE_PLAYER_GAMES = SQLValidators.VALIDATE_DUPLICATE_PLAYER_GAMES
VALIDATE_SEASON_RANGES = SQLValidators.VALIDATE_SEASON_RANGES
VALIDATE_PLAYER_CAREER_SEASONS = SQLValidators.VALIDATE_PLAYER_CAREER_SEASONS
VALIDATE_STAT_PERCENTAGES = SQLValidators.VALIDATE_STAT_PERCENTAGES_SEASON
VALIDATE_STAT_PERCENTAGES_GAME = SQLValidators.VALIDATE_STAT_PERCENTAGES_GAME
VALIDATE_GAME_SCORES = SQLValidators.VALIDATE_GAME_SCORES
VALIDATE_GAME_QUARTER_SUMS = SQLValidators.VALIDATE_GAME_QUARTER_SUMS
VALIDATE_PLAYER_AGES = SQLValidators.VALIDATE_PLAYER_AGES
VALIDATE_PLAYER_CAREER_SPAN = SQLValidators.VALIDATE_PLAYER_CAREER_SPAN
VALIDATE_TEAM_SEASON_CONTINUITY = SQLValidators.VALIDATE_TEAM_SEASON_CONTINUITY
VALIDATE_TEAM_ACTIVE_YEARS = SQLValidators.VALIDATE_TEAM_ACTIVE_YEARS
